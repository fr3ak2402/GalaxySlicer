...
const std::vector<std::pair<GCodeProcessor::EProducer, std::string>> GCodeProcessor::Producers = {
    //BBS: GalaxySlicer is also "bambu". Otherwise the time estimation didn't work.
    //FIXME: Workaround and should be handled when do removing-bambu
    { EProducer::GalaxySlicer, SLIC3R_APP_NAME },
    { EProducer::GalaxySlicer, "generated by GalaxySlicer" },
    { EProducer::GalaxySlicer, "generated by BambuStudio" },
    { EProducer::GalaxySlicer, "BambuStudio" }
...
void GCodeProcessor::process_file(const std::string& filename, std::function<void()> cancel_callback)
{
    ...
    // parse the gcode file to detect its producer
    {
        ...
        // if the gcode was produced by GalaxySlicer,
        // extract the config from it
        if (m_producer == EProducer::GalaxySlicer || m_producer == EProducer::Slic3rPE || m_producer == EProducer::Slic3r) {
            DynamicPrintConfig config;
            config.apply(FullPrintConfig::defaults());
            // Silently substitute unknown values by new ones for loading configurations from GalaxySlicer's own G-code.
...
void GCodeProcessor::process_tags(const std::string_view comment, bool producers_enabled)
{
    ...
    if (!producers_enabled || m_producer == EProducer::GalaxySlicer) {
        // height tag
...
bool GCodeProcessor::process_producers_tags(const std::string_view comment)
{
    ...
    case EProducer::GalaxySlicer: { return process_bambuslicer_tags(comment); }
