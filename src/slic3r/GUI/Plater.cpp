    ...
    : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(42 * wxGetApp().em_unit(), -1)), p(new priv(parent))
{
    ...
    StateColor flush_bd_col(std::pair<wxColour, int>(wxColour(105, 58, 113), StateColor::Pressed),
                            std::pair<wxColour, int>(wxColour(105, 58, 113), StateColor::Hovered),
...
void Sidebar::sync_ams_list()
{
    ...
    if (unknowns > 0) {
        MessageDialog dlg(this,
            _L("There are some unknown filaments mapped to generic preset. Please update GalaxySlicer or restart GalaxySlicer to check if there is an update to system presets."),
            _L("Sync filaments with AMS"), wxOK);
        dlg.ShowModal();
    }
                    ...
                    if (en_3mf_file_type == En3mfType::From_Prusa) {
                        ...
                        if (load_type != LoadType::LoadGeometry) {
                            if (en_3mf_file_type == En3mfType::From_BBS)
                                show_info(q, _L("The 3mf is generated by old GalaxySlicer, load geometry data only."), _L("Load 3mf"));
                            else
                                show_info(q, _L("The 3mf is not from Bambu Lab, load geometry data only."), _L("Load 3mf"));
                        }
...
/*#ifdef __APPLE__
    ...
    BOOST_LOG_TRIVIAL(debug) << "GalaxySlicer window going forward";
                ...
                wxCAPTION | wxCLOSE_BOX)
{
    ...
    StateColor btn_bg_green(std::pair<wxColour, int>(wxColour(0, 137, 123), StateColor::Pressed), std::pair<wxColour, int>(wxColour(156, 109, 164), StateColor::Hovered),
...
void Plater::show_object_info()
{
    ...
    if (non_manifold_edges > 0) {
        info_manifold += into_u8("\n" + _L("Tips:") + "\n" +_L("\"Fix Model\" feature is currently only on Windows. Please repair the model on GalaxySlicer(windows) or CAD softwares."));
    }
    #endif //APPLE & LINUX
