...
{
    std::string icon_path = (boost::format("%1%/images/GalaxySlicerTitle.ico") % Slic3r::resources_dir()).str();
...
,m_matrix(matrix), m_min_flush_volume(extra_flush_volume), m_max_flush_volume(Slic3r::g_max_flush_volume)
{
    ...
    {
        wxString message = _L("Galaxy would re-calculate your flushing volumes everytime the filaments color changed. You could disable the auto-calculate in GalaxySlicer  > Preferences");
