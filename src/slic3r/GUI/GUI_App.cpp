...
{
public:
    GalaxySlicerSplashScreen(const wxBitmap& bitmap, long splashStyle, int milliseconds, wxPoint pos = wxDefaultPosition)
...
/*#ifdef WIN32
    ...
    try {
        ...
        flush_logs();
        wxString errmsg = wxString::Format(_L("GalaxySlicer will terminate because of running out of memory."
        ...
        //throw;
     	...
     	BOOST_LOG_TRIVIAL(error) << boost::format("Uncaught exception: %1%") % ex.what();
        	flush_logs();
        wxString errmsg = _L("GalaxySlicer will terminate because of a localization error. "
                             "It will be appreciated if you report the specific scenario this issue happened.");
        wxMessageBox(errmsg + "\n\n" + wxString(ex.what()), _L("Critical error"), wxOK | wxICON_ERROR);
        std::terminate();
        //throw;
    } catch (const std::exception& ex) {
        wxLogError(format_wxstr(_L("GalaxySlicer got an unhandled exception: %1%"), ex.what()));
        BOOST_LOG_TRIVIAL(error) << boost::format("Uncaught exception: %1%") % ex.what();
        flush_logs();
        throw;
    }
...
void GUI_App::post_init()
{
    ...
    if (!this->init_params->input_files.empty()) {
        ...
        if (this->init_params->input_files.size() == 1 &&
            boost::starts_with(this->init_params->input_files.front(), "galaxyslicer://open")) {
            auto input_str_arr = split_str(this->init_params->input_files.front(), "galaxyslicer://open/?file=");
...
static boost::optional<Semver> parse_semver_from_ini(std::string path)
{
    ...
    size_t start = body.find("GalaxySlicer ");
    if (start == std::string::npos) {
        start = body.find("GalaxySlicer ");
        if (start == std::string::npos)
            return boost::none;
    }
...
//     Slic3r::debugf "wxWidgets version %s, Wx version %s\n", wxVERSION_STRING, wxVERSION;
        ...
        if (!msg.empty() && !ssl_accept) {
            RichMessageDialog
                dlg(nullptr,
                    ...
                    "GalaxySlicer", wxICON_QUESTION | wxYES_NO);
    ...
    if (app_config->get("show_splash_screen") == "true") {
        ...
        scrn = new GalaxySlicerSplashScreen(bmp, wxSPLASH_CENTRE_ON_SCREEN | wxSPLASH_TIMEOUT, 10000, splashscreen_pos);
#ifndef __linux__
        wxYield();
#endif
    ...
    if (is_editor()) {
            ...
            if (this->plater_ != nullptr) {
               ...
               /* wxString tips = wxString::Format(_L("Click to download new version in default browser: %s"), version_info.version_str);
                DownloadDialog dialog(this->mainframe,
                    tips,
                    _L("New version of GalaxySlicer"),
                    false,
        ...
        Bind(EVT_ENTER_FORCE_UPGRADE, [this](const wxCommandEvent& evt) {
                ...
                DownloadDialog dialog(this->mainframe,
                    tips,
                    _L("The GalaxySlicer needs an upgrade"),
                    false,
                    wxCENTER | wxICON_INFORMATION);
...
void GUI_App::on_http_error(wxCommandEvent &evt)
{
    ...
    if (code == HttpErrorVersionLimited) {
        MessageDialog msg_dlg(nullptr, _L("The version of GalaxySlicer is too low and needs to be updated to the latest version before it can be used normally"), "", wxAPPLY | wxOK);
        if (msg_dlg.ShowModal() == wxOK) {
        }
...
{
    // Galaxy: alaways disable track event
    return;
...
std::string GUI_App::format_display_version()
{
    ...
    version_display = GalaxySlicer_VERSION;
...
bool GUI_App::load_language(wxString language, bool initial)
{
	...
	if (language_info != nullptr && language_info->LayoutDirection == wxLayout_RightToLeft) {
    	BOOST_LOG_TRIVIAL(trace) << boost::format("The following language code requires right to left layout, which is not supported by GalaxySlicer: %1%") % language_info->CanonicalName.ToUTF8().data();
		language_info = nullptr;
	}
    ...
    if (! wxLocale::IsAvailable(language_info->Language)) {
    	...
    	wxString message = "Switching GalaxySlicer to language " + language_info->CanonicalName + " failed.";
        ...
        wxMessageBox(message, "GalaxySlicer - Switching language failed", wxOK | wxICON_ERROR);
        if (initial)
...
void GUI_App::OSXStoreOpenFiles(const wxArrayString &fileNames)
{
    ...
    if (fileNames.size() == num_gcodes) {
        // Opening PrusaSlicer by drag & dropping a G-Code onto GalaxySlicer icon in Finder,
