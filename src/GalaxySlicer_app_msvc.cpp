...
public:
    ...
    bool load_opengl_dll()
    {
        ...
        wc.lpszClassName = L"GalaxySlicer_opengl_version_check";
        wc.style = CS_OWNDC;
        if (RegisterClass(&wc)) {
            HWND hwnd = CreateWindowW(wc.lpszClassName, L"GalaxySlicer_opengl_version_check", WS_OVERLAPPEDWINDOW, 0, 0, 640, 480, 0, 0, wc.hInstance, (LPVOID)this);
            if (hwnd) {
...
extern "C" {
    ...
    Slic3rMainFunc galaxyslicer_main = nullptr;
}
...
// http://download.qt.io/development_releases/prebuilt/llvmpipe/windows/
    ...
    wcscat(path_to_slic3r, L"GalaxySlicer.dll");
//	printf("Loading Slic3r library: %S\n", path_to_slic3r);
    HINSTANCE hInstance_Slic3r = LoadLibraryExW(path_to_slic3r, nullptr, 0);
    if (hInstance_Slic3r == nullptr) {
        printf("GalaxySlicer.dll was not loaded, error=%d\n", GetLastError());
        return -1;
    }

    // resolve function address here
    galaxyslicer_main = (Slic3rMainFunc)GetProcAddress(hInstance_Slic3r,
#ifdef _WIN64
    ...
    if (galaxyslicer_main == nullptr) {
        printf("could not locate the function galaxyslicer_main in GalaxySlicer.dll\n");
        return -1;
    }
    // argc minus the trailing nullptr of the argv
    return galaxyslicer_main((int)argv_extended.size() - 1, argv_extended.data());
}
}
