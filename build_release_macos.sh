...
mkdir -p build_$ARCH
cd build_$ARCH
DEPS=$PWD/GalaxySlicer_dep_$ARCH
mkdir -p $DEPS
...
then
    ...
    if [ "1." == "$PACK_DEPS". ];
    then
        tar -zcvf /$WD/build_$ARCH/GalaxySlicer/GalaxySlicer_dep_mac_${ARCH}_$(date +"%Y%m%d").tar.gz GalaxySlicer_dep_$ARCH
    fi
fi
...
cmake .. -GXcode -DBBL_RELEASE_TO_PUBLIC=1 -DCMAKE_PREFIX_PATH="$DEPS/usr/local" -DCMAKE_INSTALL_PREFIX="$PWD/GalaxySlicer" -DCMAKE_BUILD_TYPE=Release -DCMAKE_MACOSX_RPATH=ON -DCMAKE_INSTALL_RPATH="$DEPS/usr/local" -DCMAKE_MACOSX_BUNDLE=ON -DCMAKE_OSX_ARCHITECTURES=${ARCH} -DCMAKE_OSX_DEPLOYMENT_TARGET=${OSX_DEPLOYMENT_TARGET}
...
cp -pR ../src/Release/GalaxySlicer.app ./GalaxySlicer.app
resources_path=$(readlink ./GalaxySlicer.app/Contents/Resources)
rm ./GalaxySlicer.app/Contents/Resources
cp -R $resources_path ./GalaxySlicer.app/Contents/Resources
